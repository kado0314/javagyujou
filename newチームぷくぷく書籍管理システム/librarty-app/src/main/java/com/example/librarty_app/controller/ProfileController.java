...
public class ProfileController {
    ...
    public String editConfirm(
        ...
        // パスワードが入力されている場合、バリデーションエラーチェック
        if (userEditForm.getPassword() != null && !userEditForm.getPassword().isEmpty()) {
             // パスワードの最小文字数チェックはUserEditFormのアノテーションで処理される
        } else {
             // パスワードが空の場合、バリデーションエラーを無視するために手動でチェック
             // ここでは、パスワードが空の場合は既存のパスワードを維持するものと想定し、
             // その他のフィールドのエラーのみをチェックする
             if (result.hasFieldErrors("email") || result.hasFieldErrors("nickname") || result.hasFieldErrors("zipcode") || result.hasFieldErrors("address") || result.hasFieldErrors("phone")) {
                 redirectAttributes.addFlashAttribute("errorMessage", "入力内容に不備があります。再度確認してください。");
                 return "redirect:/profile/edit";
             }
        }
        
        if (result.hasErrors() && (userEditForm.getPassword() != null && !userEditForm.getPassword().isEmpty())) {
             // パスワードが入力されていて、かつエラーがある場合はエラーメッセージを表示
             redirectAttributes.addFlashAttribute("errorMessage", "入力内容に不備があります。再度確認してください。");
             return "redirect:/profile/edit";
        }
    ...
    // ... (showLogoutConfirm, logout, showDeleteConfirm のメソッドは省略) ...
    ...
    // ログアウトと削除の確認画面へのGETマッピング（URL導線集約のため復元）
